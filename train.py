...
for i in range(1, level):
    ...
    for phase in ['fade_in', 'stabilize']:
        ...
        for epoch in range(num_epochs):
            ...
            mixing_factors = torch.rand(x_real.size(0), 1, 1, 1)
            if cuda:
                mixing_factors = mixing_factors.cuda()
            x_mix = Variable((x_real.data * (1 - mixing_factors) + x_fake.data * mixing_factors), requires_grad=True)
            ...
            print('Resolution:%d, %s, iter[%d|%d], D_loss: %.4f, G_loss: %.4f, elapsed_time: %.2fsec.' % 
                (resolution, phase, epoch, num_epochs, d_loss.data[0], g_loss.data[0], elapsed_time))
